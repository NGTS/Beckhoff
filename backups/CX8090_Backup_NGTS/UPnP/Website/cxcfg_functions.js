...
function write_plc_val(name, value, size, type)
{
    ...
    // Fix length of value for values of type bool
    if (type == "BOOL" || type == "bool")
        vallen = 1;
    else
        vallen = value.length;

    ndata = prepare_data(name.length + vallen + 12, "int", 4, ndata);
    ndata = prepare_data(name.length, "int", 4, ndata);
    ndata = prepare_data(vallen, "int", 4, ndata);
...
function doReboot(need_confirm)
{
    ...
    if (answer != 0)
    {
        if (rebootindex == -1) {
            clear_info();
            show_info("Unable to reboot", "red");
            return;
        }
        ...
        var qData = new QueryData(rebootindex, 1, 0, 0, "uint", 1, "reboot", true);
...
{
    var qData = new QueryData(mod.index + 1, 5, 0, 4, "uint", value, "", false);
    new Mutex(query_write,qData,0);
}
...
function mbloop() {
    if (mod.type == "MAINBOARD") {
        ...
        new Mutex(query_readmultiple, new QueryData(idx, sidx, flag, len, type, i, "MAINBOARD", true), 1);
    }
}
...
function fanloop() {
    if (mod.type == "FAN") {
        ...
        new Mutex(query_readmultiple, new QueryData(idx, sidx, flag, len, type, i, "FAN", true), 1);
    }
}
...
function runWriteTrigger(i) {
    debugln("<strong>RunWriteTrigger: </strong>" + i);

    var idx = mod.writeTriggers[i].index;
    var sidx = mod.writeTriggers[i].subindex;
    var type = mod.writeTriggers[i].type;
    var len = mod.writeTriggers[i].len;
    var value = mod.writeTriggers[i].value;

    var id = "wt" + i;
    if (mod.writeTriggers[i].id != "")
        id = mod.writeTriggers[i].id;

    var qData = new QueryData(idx, sidx, 0, len, type, value, id, false);
    new Mutex(query_write, qData, 1);
}
...
function fixLoadBoxPos() {
    // Alle Browser || IE Standardmodus || IE Quirksmodus   
    var width = window.innerWidth || (window.document.documentElement.clientWidth || window.document.body.clientWidth);                 
    var height = window.innerHeight || (window.document.documentElement.clientHeight ||window.document.body.clientHeight);

    //  Netscape || Dom || IE6
    var scrollBarX = window.pageXOffset || document.body.scrollLeft || document.documentElement.scrollLeft;
    var scrollBarY = window.pageYOffset || document.body.scrollTop || document.documentElement.scrollTop;

    document.getElementById("load_box").style.top = ((height / 2) + scrollBarY - 30) + "px";
    document.getElementById("load_box").style.left = ((width / 2) + scrollBarX - 125) + "px";
}
