...
function format_add(type, id, disable) {
    ...
    switch (type) {
        ...
        case "routeflags":
            ...
            text += "<input type=\"checkbox\" id=\"c_temp\" onclick=\"javascript:routeflags('"
                + id + "')\"> Temporary <br>";
            text += "<input type=\"checkbox\" id=\"c_dyn\" onclick=\"javascript:routeflags('"
                + id + "')\"> Dynamic <br>";
            break;
        ...
        case "ar_form":
            ...
            text += "<option value=\"0\">USER_DELETE</option>";
            text += "<option value=\"10\">SHARE_READ</option>";
            text += "<option value=\"20\">SHARE_CHANGE</option>"
            text += "<option value=\"-1\">SHARE_FULL</option>";
            text += "</select>";
        ...
        case "groupdom":
            text = "<input type=\"text\" id=\"" + id + "\" value=\"" + general_info.hostname + "\" class=\"input\" type=\"text\" style=\"width:98%;\" DISABLED>";
            break;

        case "groupmembership":
            text = "<input type=\"hidden\" id=\"" + id + "\" value=\"1\">";
            text += "<span><input type=\"radio\" id=\"c_addgrpmem\" name=\"groupmembership\" onChange=\"javascript:groupMembership('" + id + "');\" checked />&nbsp;Add</span>"
            text += "<span style=\"margin-left:5px\"><input type=\"radio\" id=\"c_rmgrpmem\" name=\"groupmembership\" onChange=\"javascript:groupMembership('" + id + "');\" />&nbsp;Remove</span>";
            break;
...
function custom_value_hidden_input_add(type, id) {
    var customVal = "";
    var text = "";

    switch (type) {
        case "groupdom":
            customVal = general_info.hostname;
            break;
    }

    return "<input id=\"" + id + "\" value=\"" + customVal + "\" class=\"input\" type=\"hidden\" />";
}
...
function groupMembership(id) {
    var add = document.getElementById("c_addgrpmem");
    var rm = document.getElementById("c_rmgrpmem");
...
function format(j, value, i, id) {
    ...
    switch (mod.variables[j].displaytype) {
        ...
        case "ar":
            ...
            var tmp = "";
            for (var k = 0; k < ar_values.length; k++) {
                switch (ar_values[k]) {
                    case "0":
                        tmp += (k + 1) + ": " + "USER_DELETE";
                        break;
        ...
        case "UPSBatteryStatus":
            ...
            switch (value) {
                ...
                case "1": // = OK
                    value = "OK";
                    break;
                case "2": // = Change Battery 
                    value = "Change Battery";
                    break;
            }
            break;


        case "MbVInfoLocation":

            switch (value) {

                case "0": // = LOCATION_UNKNOWN
                    value = "UNKNOWN";
                    break;
                case "1": // = LOCATION_OTHER
                    value = "OTHER";
                    break;
                case "2": // = LOCATION_PROCESSOR
                    value = "PROCESSOR";
                    break;
                case "3": // = LOCATION_DISK
                    value = "DISK";
                    break;
                case "4": // = LOCATION_SYSTEM_MANAGEMENT_MODULE
                    value = "SYSTEM_MANAGEMENT_MODULE";
                    break;
                case "5": // = LOCATION_MOTHERBOARD
                    value = "MOTHERBOARD";
                    break;
                case "6": // = LOCATION_MEMORY_MODULE
                    value = "MEMORY_MODULE";
                    break;
                case "7": // = LOCATION_POWER_SUPPLY
                    value = "POWER_SUPPLY";
                    break;
                case "8": // = LOCATION_ADDIN_CARD
                    value = "ADDIN_CARD";
                    break;
                case "9": // = LOCATION_FRONT_PANEL_BOARD
                    value = "FRONT_PANEL_BOARD";
                    break;
                case "10": // = LOCATION_BACK_PANEL_BOARD
                    value = "BACK_PANEL_BOARD";
                    break;
                case "11": // = LOCATION_PERIPHERIE
                    value = "PERIPHERIE";
                    break;
                case "12": // = LOCATION_CHASSIS
                    value = "CHASSIS";
                    break;
                case "13": // = LOCATION_BATTERY
                    value = "BATTERY";
                    break;
                case "14": // = LOCATION_UPS
                    value = "UPS";
                    break;
                case "15": // = LOCATION_GRAFFIC_BOARD
                    value = "GRAFFIC_BOARD";
                    break;
                case "16": // = LOCATION_SUPERIO;
                    value = "SUPERIO"
                    break;
                case "17": // = LOCATION_CHIPSET
                    value = "CHIPSET";
                    break;
        ...
        case "groupmember":
            ...
            var grpArr = value.split(";");
            var newVal = "";
            if (grpArr.length > 1) {
                for (var k = 0; k < grpArr.length; k++) {
                    if (k < grpArr.length - 1)
                        newVal += grpArr[k] + ",<br />";
                    else
                        newVal += grpArr[k];
                }
...
function apply(subindex) {
    ...
    for (var i = 0; i < mod.variables.length; i++) {
        ...
        try {
            ...
            clear_info();
            load_end();
            show_info("Unexpected error", "red");
            value = "";
        }
    ...
    new Mutex(load_end, null, -1, "applyFinished");
}
    ...
    for (var i = 0; i < mod_count; i++) {
        ...
        if (!mod_list[i].hide) {
            ...
            if ((count_types(mod_list[i].type) > 1 && current_list != mod_list[i].type)) {
                ...
                if (current_customFolder == "" || current_customFolder != mod_list[i].customFolder) {
                    text += "<tr><td valign=\"top\"><table width=\"100%\" class=\"menu\" cellspacing=\"0\" cellpadding=\"1\"><tr><td width=\"60\" valign=\"top\" nowrap><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><a href=javascript:collapse('"
                            + i
                            + "')><img alt=\"\" id=\"micon_"
                            + i
                            + "\" border=\"0\" src=\"IMAGES/MU.gif\"><img alt=\"\" border=\"0\" src=\"IMAGES/OZ.gif\"></a></td><td>"
                ...
                {
                    text += "<tr><td valign=\"top\"><table width=\"100%\" class=\"menu\" cellspacing=\"0\" cellpadding=\"1\"><tr><td width=\"80\" valign=\"top\" nowrap><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><a href=javascript:collapse('"
                            + i
                            + "')><img alt=\"\" id=\"micon_"
                            + i
                            + "\" border=\"0\" src=\"IMAGES/MU.gif\"><img alt=\"\" border=\"0\" src=\"IMAGES/OZ.gif\"></a></td><td>"
            ...
            if ((count_types(mod_list[i].type) > 1 && current_list == mod_list[i].type || mod_list[i].customFolder != "" && current_customFolder == mod_list[i].customFolder)) {
                ...
                if (current_customFolder != "" && current_list == mod_list[i].type && current_customFolder == mod_list[i].customFolder) {
                    text += "<tr><td valign=\"top\"><table width=\"100%\" cellspacing=\"0\" cellpadding=\"1\" class=\"menu\"><tr><td width=\"100\" id=\"m"
                            + i
                            + "i\" valign=\"top\" nowrap><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/SE.gif\"></td><td id=\"m"
                            + i
                            + "\"><div style=\"overflow:hidden; width:120px; \"><a href=\""
                            + fixLink("index.htm#" + i)
                            + "\">"
                            + nameN
                ...
                else // leaf on level 2
                {
                    text += "<tr><td valign=\"top\"><table width=\"100%\" cellspacing=\"0\" cellpadding=\"1\" class=\"menu\"><tr><td width=\"80\" id=\"m"
                            + i
                            + "i\" valign=\"top\" nowrap><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/SE.gif\"></td><td id=\"m"
                            + i
                            + "\"><div style=\"overflow:hidden; width:140px; \"><a href=\""
                            + fixLink("index.htm#" + i)
                            + "\">"
                            + nameN
                            + "</a></div></td></tr></table></td></tr>";
            ...
            else // Print leaf on level 1
            {
                text += "<tr><td valign=\"top\"><table width=\"100%\" class=\"menu\" cellspacing=\"0\" cellpadding=\"1\"><tr><td width=\"60\" id=\"m"
                        + i
                        + "i\" valign=\"top\" nowrap><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/LU.GIF\"><img alt=\"\" border=\"0\" src=\"IMAGES/SE.gif\"></td><td id=\"m"
                        + i
                        + "\"><div style=\"overflow:hidden; width:160px; \"><a href=\""
                        + fixLink("index.htm#" + i)
                        + "\">" + nameN + "</a></div></td></tr></table></td></tr>";
            }
        }
    }
