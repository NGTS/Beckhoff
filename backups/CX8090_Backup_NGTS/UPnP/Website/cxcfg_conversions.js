...
function eval_data(k, i, text, len, type)
{
    ...
    else
    {
        if (type == "int") {
            ...
            switch (len) {
                case 4:
                    ...
                    break;
                case 2:
                    var cCode = (text.charCodeAt(k + 1) << 8 | text.charCodeAt(k));
            ...
            switch (len) {
                case 8:
                    ...
                    for (var h = 0; h < 8; ++h) {
                        hBytes[h] = text.charCodeAt(k + h).toString(16).toUpperCase();
                        if (hBytes[h].length == 1) {
                            hBytes[h] = "0" + hBytes[h];
                        }
                    }
                    ...
                    break;
                case 4:
                    c_val = (text.charCodeAt(k + 3) << 24 | text.charCodeAt(k + 2) << 16 | text.charCodeAt(k + 1) << 8 | text.charCodeAt(k)) >>> 0; // ">>> 0" = handle value as unsigned
                    break;
                case 2:
                    c_val = (text.charCodeAt(k + 1) << 8 | text.charCodeAt(k)) >>> 0; // ">>> 0" = handle value as unsigned
                    break;
                case 1:
                    c_val = text.charCodeAt(k) >>> 0; // ">>> 0" = handle value as unsigned
                    break;
            }
            k = k + len;
        }
        ...
        else if (type == "ar_string") {

            //-----------------------------
            // This type is used for access rights string of SMB module only;
            //-----------------------------

            // Get userlist response from previous request and count user entries in userlist
            var ul_req = requests.get(0)[i-1];
            var ul_data = eval_data(k - ul_req.len, i - 1, text, ul_req.len, ul_req.type);
            var ul_str = ul_data.c_val;
            var ul_arr = ul_str.split(';');
            var ul_cnt = ul_arr.length;

            var c_val = [];
            for (var tmp_ul_i = 0; tmp_ul_i < ul_cnt; tmp_ul_i++) {
                var tmp_byte_str = "";
                for (var tmp_byte_i = 0; tmp_byte_i < 4; tmp_byte_i++) {
                    tmp_byte_str += text.charAt(k);
                    k++;
                }
                var tmp_ed = eval_data(0, 0, tmp_byte_str, 4, "uint");
                c_val[tmp_ul_i] = tmp_ed.c_val;
            }
...
function parse_plc_vars(data, prefix)
{
    ...
    default:
        clear_info();
        show_info("Invalid PLC-Object Version: " + version, "red");
        load_end();
        return;
    }
...
function parse_ds(data, mode)
{
    ...
    else if (version != 1)
    {
        if (mode != "FAVG") {
            clear_info();
            show_info("Invalid DS-Object Version: " + version, "red");
        }
...
function parse_dirs(data)
{
    ...
    if (version != 0) {
        clear_info();
        show_info("Invalid File-Object Version: " + version, "red");
        load_end();
        return;
    }
