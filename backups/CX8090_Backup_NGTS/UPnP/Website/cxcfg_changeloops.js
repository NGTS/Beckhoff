function processReqChangeLoop() {
    if (lreq.readyState == 4) {
        clearTimeout(lreq_timeout_handle); // Clear timeout handle
        try {
            if (lreq.status == 200 || lreq.status == 304) {
                ...
                if (requests.getItem(0, 1).id == "reboot") reboottext = "Initializing Network";
                    ...
                    load_end();
                    clear_info();
                    show_info("Query failed with request status \"" + lreq.status + "\".", "red");
                }
            ...
            load_end();
            clear_info();
            show_info("Query failed!", "red");
            show_info("Description: " + e, "red");
        }
    }
}
...
function processReqChange() {
    if (req.readyState == 4) {
        ...
        clearTimeout(req_timeout_handle); // Clear timeout handle
        try {
            if ((req.status == 200 || req.status == 304) && !hasFailed) {
                ...
                try {
                    ...
                    try {// read / readwrite
                        ...
                        if (c_requests[0].index == rebootindex) {
                            return;
                        }
                        ...
                        if (c_requests[0].id == "wtUPSTestCapacity") {
                            clear_info();
                            show_info("WriteTrigger for \"Test Capacity\" successfully sended!", "green");
                            return;
                        }
                    ...
                    if ((c_requests[0].index == browseindex + 4) && c_requests[0].subindex == 1) {
                        if (c_requests[0].id == 'GetFavVals')
                            parse_plc_vars(text, "f");
                        else {
                            parse_plc_vars(text, "");
                            load_end();
                        }
                        ...
                        if (c_requests[0].subindex == 2) {
                            ...
                            if (state != 1 && state != 0) {
                                if (state == 3) {
                                    ...
                                    debugln("<b>Loading</b> - State: " + state);
                                    errors++;
                            ...
                            if (errCode == -324597584) {
                                show_info("The user account already exists.", "red");
                            } else {
                                show_info("Command failed.", "red");
                                show_info("Errorcode: " + errCode);
                            }
                    ...
                    if (c_requests[0].name == "WriteMultiple") {
                        for (i = 0; i < c_requests.length; i++) {
                            ...
                            if (a_err[i] != 0) {
                                clear_info();
                                show_info("Unable to change " + mod.get_request_variable(i).name, "red");
                                show_info("Errorcode: " + a_err[i], "red");
                            }
                        ...
                        if (c_requests[0].id == "apply")
                            Mutex.notify("applyFinished");
                    ...
                    if (c_requests[0].name == "ReadMultiple" && c_requests[0].index != 0xf020) {
                        ...
                        for (i = 0; i < c_requests.length; i++) {
                            ...
                            if (c_requests[i].index % 16 == 0) { // type object
                                if (c_requests[i].subindex == 3) { // name
                                    mod_list[mod_count++] = new Module(c_val, lasttype, c_requests[i].index, c_requests[i].type);
                                    debugln(c_val + "," + lasttype + ";");

                                    if (lasttype == "DataStore" && c_val == "System")
                                        systemdsindex = c_requests[i].index + 12288;
                                ...
                                if (c_requests[i].id == "init") {
                                    if (c_requests[i].index == 0x1008) {
                                        ...
                                        if (!c_val.startsWith("??????;v??.?") && c_val != "??0000;v00.0;0000-00-00" && c_val != "??0000;v00.0;2000-00-00") {
                                            var giHwStr = "";
                                            ...
                                            if (giHwArr[2] && giHwArr[2] != "0000-00-00" && giHwArr[2] != "2000-00-00" && giHwArr[2] != "????-??-??") {
                                                ...
                                                var giHwDateYearTmp = parseFloat(giHwDateYear);
                                                if (giHwDateYearTmp >= 2000) {
                                                    giHwDateYear = giHwDateYearTmp;
                                                }
                                                else {
                                                    giHwDateYear = giHwDateYearTmp + 2000;
                                                }
                                    ...
                                    if (mod.variables[t].editable) {
                                        ...
                                        if (!a_err[i]) {
                                            ...
                                            var tmpData = format(t, c_val, -1, prefix + c_requests[i].subindex + "," + mod.variables[t].index);
                                            document.getElementById(prefix + c_requests[i].subindex + "," + mod.variables[t].index).innerHTML = tmpData;

                                            for (var x = 0; x < mod.arrayComboBoxIndex; x++) {
                                                if (mod.arrayComboBoxIndex && mod.arrayComboBoxSubIndex && mod.arrayComboBoxIndex[x] == mod.variables[t].index && mod.arrayComboBoxSubIndex[x] == mod.variables[t].subindex) {
                                                    // ComboBox
                                                    document.getElementById("option" + c_requests[i].subindex + "," + mod.variables[t].index).innerHTML = tmpData;
                                                    // ContentHeader
                                                    document.getElementById("headerContent" + c_requests[i].subindex + "," + mod.variables[t].index).innerHTML = tmpData;
                                                }
                                            }

                                            // Function Links
                                            if (mod.functionLinks && mod.functionLinks.length > 0) {
                                                for (var x = 0; x < mod.functionLinks.length; x++) {
                                                    if (mod.functionLinks[x].targetIndex == c_requests[i].index && mod.functionLinks[x].targetSubIndex == mod.variables[t].subindex) 
                                                    {
                                                        var funcLinkSelect = document.getElementById(
                                                        "a" + mod.functionLinks[x].funcVarName + mod.functionLinks[x].funcIndex
                                                        );
                                                        funcLinkSelect.add(new Option(tmpData, tmpData, false, false));
                                                    }
                                                }
                ...
                debugln("Query failed with status <b>" + req.status + "</b>");
                load_end();
                clear_info();
                errors++;
                ...
                try {
                    ...
                    show_info("Query failed with error code \"" + errorCode + "\".", "red");
                    show_info(errorDescription);
                }
                catch (e) {
                    show_info("Query failed with request status \"" + req.status + "\".", "red");
                }
            }
        } catch (e) {
            load_end();
...
function lreqTimeout() {
    if (requests.getItem(0, 1).id == "reboot") return;
    lreq.abort();
}